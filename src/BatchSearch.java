...
public class BatchSearch {
	...
	public static void main(String[] args) throws Exception {
		...
		if ("default".equals(simstring)) {
			...
			simfn = new BM25Similarity(1.2f,0.6f);
		} else if ("dfr".equals(simstring)) {
			simfn = new DFRSimilarity(new BasicModelP(), new AfterEffectL(), new NormalizationH2());
		} else if ("lm".equals(simstring)) {
			simfn = new LMDirichletSimilarity();
		} else if ("my".equals(simstring)) {
			simfn = new MySimilarity();
		}
		if (simfn == null) {
			System.out.println(usage);
			System.out.println("Supported similarity functions:\ndefault: DefaultSimilary (tfidf)");
		...
		while (true) {
			...
			//expandQuery(searcher, reader, query, field);
			doBatchSearch(in, searcher, pair[0], query, simstring);
		}
		reader.close();
	}

	public static void expandQuery(IndexSearcher searcher,IndexReader reader,Query query,String field) throws IOException{
	
		TopDocs tenResults = searcher.search(query, 10);
		ScoreDoc[] tenHits = tenResults.scoreDocs;
		int actHits =tenResults.totalHits;
		
		for(int j=0;j < Math.min(actHits, 10); j++){
			//Document doc = searcher.doc(tenHits[j].doc);
			//String docno = doc.get("docno");
			Terms topTerm = reader.getTermVector(tenHits[j].doc, field);
			StringBuilder sb = new StringBuilder();
			TermsEnum termEnum  = topTerm.iterator(null);
			BytesRef text;
			while((text = termEnum.next()) != null) {
					//System.out.println("text=" + text.utf8ToString() + "; freqCount= " + r +  "; freqCountcontent = " + r1  +"; freqCountdesc= " + r2 );
